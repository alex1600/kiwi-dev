...
namespace KiwiAttributes.Editor
{
	[CanEditMultipleObjects]
	[CustomEditor(typeof(UnityEngine.Object), true)]
	public class KiwiInspector : UnityEditor.Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			bool anyKiwiAttribute = _serializedProperties.Any(p => PropertyUtility.GetAttribute<IKiwiAttribute>(p) != null);
			if (!anyKiwiAttribute)
			{
		...
		protected void DrawSerializedProperties()
		{
			...
			foreach (var property in GetNonGroupedProperties(_serializedProperties))
			{
				...
				{
					KiwiEditorGUI.PropertyField_Layout(property, includeChildren: true);
				}
			}
			...
			foreach (var group in GetGroupedProperties(_serializedProperties))
			{
				...
				KiwiEditorGUI.BeginBoxGroup_Layout(group.Key);
				foreach (var property in visibleProperties)
				{
					KiwiEditorGUI.PropertyField_Layout(property, includeChildren: true);
				}

				KiwiEditorGUI.EndBoxGroup_Layout();
			}
			...
			foreach (var group in GetFoldoutProperties(_serializedProperties))
			{
				...
				if (_foldouts[group.Key].Value)
				{
					foreach (var property in visibleProperties)
					{
						KiwiEditorGUI.PropertyField_Layout(property, true);
					}
				}
			}
		...
		protected void DrawNonSerializedFields(bool drawHeader = false)
		{
			if (_nonSerializedFields.Any())
			{
				...
				{
					KiwiEditorGUI.NonSerializedField_Layout(serializedObject.targetObject, field);
				}
			}
		}
		...
		protected void DrawNativeProperties(bool drawHeader = false)
		{
			if (_nativeProperties.Any())
			{
				...
				{
					KiwiEditorGUI.NativeProperty_Layout(serializedObject.targetObject, property);
				}
			}
		}
		...
		protected void DrawButtons(bool drawHeader = false)
		{
			if (_methods.Any())
			{
				...
				{
					KiwiEditorGUI.Button(serializedObject.targetObject, method);
				}
			}
		}
